from A_star import multiple_routes_A_star, read_file_routes, mark_areas_in_grid
from utils import fitness_function, delete_file
import numpy as np
import random
import copy
...
def set_parameters_GA(file_name: str, population_coef = 2, rounds_coef = 3, parents_coef = 35, crossover_1P_coef = 10, mutations_coef = 10):
    global routes, nr_needed_routes
    routes, colors = read_file_routes(file_name=file_name, draw = False)
    nr_needed_routes = len(routes)
    blocked_areas = None

    global ROWS, COLUMNS, POPULATION_SIZE, ROUNDS, PARENTS_KEPT, CROSSOVERS_1P, MUTATIONS, OFFSET_X, OFFSET_Y
...
def genetic_routing(file_name):
    ...
    for i in range(ROUNDS):
        ...
        #for j in range(POPULATION_SIZE):
        #    file1.write(f"{i}, {j}, {next_generation[j].order}, {next_generation[j].total_cost}\n")
...
def run_genetic_algorithm(save_file, read_file, event=None):
    if not event or not event.is_set():
        ...
        genetic_routing(file_name = save_file)
